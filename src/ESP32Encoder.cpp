...
ESP32Encoder::ESP32Encoder(int16_t CPR, int32_t positionComputeInterval, bool always_interrupt_, enc_isr_cb_t enc_isr_cb, void *enc_isr_cb_data) : CPR{CPR}, positionComputeInterval{positionComputeInterval}, always_interrupt{always_interrupt_}, aPinNumber{(gpio_num_t)0}, bPinNumber{(gpio_num_t)0}, unit{(pcnt_unit_t)-1}, countsMode{2}, count{0}, r_enc_config{}, _enc_isr_cb(enc_isr_cb), _enc_isr_cb_data(enc_isr_cb_data), attached{false}, direction{false}, working{false}, wheelDiameter{15.4}, lastCountTime{0.0} {
...
int64_t ESP32Encoder::getLastCount() {
    return lastCount;
}

double ESP32Encoder::getRPM() {
    if (millis() - this->lastCountTime < this->positionComputeInterval) {
        return lastRPM;
    }

    unsigned long dt_omega = millis() - this->lastCountTime;
    int changeTicks = this->getCount() - this->getLastCount();
    double rpm = (changeTicks * (60000.0 / dt_omega)) / (float)CPR;
...
void ESP32Encoder::setWheelDiameter(float value) {
    if (value <= 0) return;
